...
export class EditBudgetEntryComponent implements OnInit {
  ...
  public constructor(
    public dialogRef: MatDialogRef<EditBudgetEntryComponent>,
  ...
  ngOnInit(): void {
    ...
    this.budgetEntryForm
      .get('dateBought')
      ?.setValue(new Date(this.budgetEntry.dateBought));
  ...
  public onSubmit(): void {
    ...
    this.budgetEntry.dateBought = this.budgetEntryForm
      .get('dateBought')
      ?.value.toISOString();

    this.store.dispatch(
      BudgetEntryActions.updateBudgetEntry({ budgetEntry: this.budgetEntry })
    );
  }
}
